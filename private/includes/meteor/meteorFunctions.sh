function buildMeteor() {
  ...
  find . -name "index.html" -type f -print0 | xargs -0 gsed -i 's#%22DDP_DEFAULT_CONNECTION_URL%22%3A%22'"${URIENC_TARGET_PROTOCOL}"'%2F%2F'"${URIENC_TARGET_HOSTNAME}"'%22#%22DDP_DEFAULT_CONNECTION_URL%22%3A%22'"${URIENC_TARGET_PROTOCOL}"'%%2F%2F'"${URIENC_TARGET_HOSTNAME}${URIENC_TARGET_PATH}"'%22#g'
}
